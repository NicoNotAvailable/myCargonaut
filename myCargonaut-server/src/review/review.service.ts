  ...
  async createReview(
    ...
    if (trip instanceof OfferTripDB) {
      ...
      if (drive.user.id !== user.id && trip.requesting.id !== user.id) {
        throw new Error('User is not authorized to write a review for this offer trip');
      }
      ...
      if (drive.user.id !== user.id && trip.requesting.id !== user.id) {
        throw new Error('User is not authorized to write a review for this request trip');
      } else if (drive.status !== StatusEnum.finished) {
        throw new BadRequestException('ride is not finished yet!')
      }
    } else {
